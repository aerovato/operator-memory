...
1. If `.operator-shared/references/effect-smol` is missing, clone `https://github.com/Effect-TS/effect-smol` there.
2. Search `.operator-shared/references/effect-smol` for exact APIs, examples, tests, and naming patterns before answering or implementing Effect-specific code.
...
- Do not answer from memory. Verify against `.operator-shared/references/effect-smol` or nearby code first.
...
- Run tests from package directories such as `packages/helper`; never run package tests from the repo root.
