...
if [ -z "$requested_version" ]; then
  printf '%s\n' 'Usage: bun run publish:pi -- <major|minor|patch|version>' >&2
  exit 1
fi

current_version=$(node -p 'require("./packages/pi/package.json").version')
version=$(sh scripts/resolve-publish-version.sh "$current_version" "$requested_version")

tag="pi@v$version"
package="@aerovato/operator-pi"
...
bun run check packages/pi
bun run build:pi
(cd packages/pi && npm pack --dry-run)

npm version "$version" --no-git-tag-version --workspaces=false --prefix packages/pi
bun install --lockfile-only --ignore-scripts

git add "bun.lock" "packages/pi/package.json"
git commit -m "pi@v$version"
git tag "$tag"
